...
void SD_Init() {
  ...
  SD_D3_EN();
  if (SD_MMC.begin("/sdcard", true, false, 40000)) {  // 40 MHz for faster reads
  ...
  if(cardType == CARD_NONE){
    ...
    printf("\n=== SD Card /assets Directory ===\n");
    File assetsDir = SD_MMC.open("/assets");
    if(!assetsDir){
      printf("Failed to open /assets directory - IT DOESN'T EXIST!\n");
    } else {
      File file = assetsDir.openNextFile();
      while(file){
        if(file.isDirectory()){
          printf("  DIR : %s\n", file.name());
        } else {
